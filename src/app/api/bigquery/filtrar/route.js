...
// async function getSchema(project, dataset, taBD_ENVIOS_SAYA_20251003project}.${dataset}.${table}`;
//   if (schemaCache.BD_ReingresosJuntoemaCache.get(key);
...
export async function POST(req) {
  try {
    ...
    const seg = filters?.find(f => f.type === 'segmento')?.value || [];
    const ase = filters?.find(f => f.type === 'asesor')?.value || [];
    ...
    const sql = `
      ...
      SELECT cod_asociado AS Codigo_Asociado,
             ...
             Telf_SMS AS celular,
             Segmento,
             E_mail AS email,  -- ✅ CAMBIAR: Email → E_mail (pero alias sigue siendo 'email')
             Zona,
             Asesor AS gestor,
             Producto
      FROM ranked_data
